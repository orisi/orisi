...
class OracleTests(unittest.TestCase):
  ...
  def test_reject_task_more_sigs(self):
    message = create_message()
    request = ('TransactionRequest', message)
    HandledTransaction(self.db).save({"txid": TXID, "max_sigs": 3})
  ...
  def test_accept_task_same_sigs(self):
    message = create_message()
    request = ('TransactionRequest', message)
    HandledTransaction(self.db).save({"txid": TXID, "max_sigs":2})
  ...
  def test_update_task_less_sigs(self):
    message = create_message()
    request = ('TransactionRequest', message)
    HandledTransaction(self.db).save({"txid": TXID, "max_sigs":1})

    self.oracle.handle_request(request)
    tasks = self.oracle.get_tasks()
    self.assertEqual(len(tasks), 1)

    self.assertEqual(HandledTransaction(self.db).signs_for_transaction(TXID), 2)
